...
def retrieve_from_vectordb(query):
    ...
    try:
        ...
        # Merge results
        # Sort the merged results and sort by reranked scores
        sorted_indices = reranked_results.argsort()[::-1]
        merged_results = {
            "documents": [retrieved_documents[i] for i in sorted_indices],
            "embeddings": [retrieved_embeddings[i] for i in sorted_indices],
            "reranked_scores": [reranked_results[i] for i in sorted_indices]
        }

        # Get the embeddings for the entire collection
        embeddings = collection.get(include=['embeddings'])['embeddings']
        query_embedding = embedding_function([query])[0]
        rerank_embeddings = merged_results['embeddings'][:5]

        # Set up UMAP for dimensionality reduction
        umap_transform = umap.UMAP(random_state=0, transform_seed=0).fit(embeddings)
        projected_dataset_embeddings = project_embeddings(embeddings, umap_transform)
        projected_query_embedding = project_embeddings([query_embedding], umap_transform)
        projected_retrieved_embeddings = project_embeddings(rerank_embeddings, umap_transform)
...
if __name__ == "__main__":
    ...
    for result in results['reranked_scores']:
        print("======================")
